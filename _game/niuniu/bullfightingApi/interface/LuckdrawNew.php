...
// 		var_dump($remTime);
		if ($actType == "lPrize") {
			...
			if ($remTime[0] <= 0) {
				if($remTime[2]==0){
					$this->returnError ( 301, '今天已经没有机会了！请明日再来吧^^' );
				}else{
					$this->returnError ( 301, '~每日最多可获得5次抽奖机会，还有'.$remTime[2].'次机会，立即获得！' );
				}				
				return;
			}	
			// 抽奖结果		
			$lotteryRes = $this->calLotteryRes ( $this->uid, $lotteryPrize,$remTime[0]);
...
// 		var_dump($resArr);
	...
	public function getRemainTime($pExt, $isSameDay, $now ,$isRes) {
		$totRec = $this->data_redis->hGet ( "hpe:{$this->uid}", "todayRechargeTot");
		$totNum = 0;
		if($totRec>0 && $totRec<20){
			$totNum = 1;
		}elseif ($totRec>=20 && $totRec<100){
			$totNum = 3;
		}elseif ($totRec>=100){
			$totNum = 5;
		}
...
// 		echo "isVip:".$isvip."<br/>";
		$rt = array($totNum+$isMore,5,5-$totNum);

		$rt[0] = $pExt ['currLotteryNum']>$rt[0]?0:$totNum - $pExt ['currLotteryNum']+$isMore;
		return $rt;
	}
