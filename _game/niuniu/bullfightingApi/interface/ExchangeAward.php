...
class ExchangeAward extends APIBase {
	...
	public function logic() {
		...
		$mob = 0;
		if ($award ['t'] != 3) {
			$hpe = $this->data_redis->hMget ( "hu:{$this->uid}", array ("mobile"));
			if (! isset ( $hpe ["mobile"] ) || strlen ( $hpe ["mobile"] ) != 11) {
		...
		$msg = "兑换".$award ['na']."成功,系统会在三个工作日内处理完毕!";
		...
		if ($curr_coin < $award ['c']) {
			$this->returnError ( 301, '元宝不足,请先充值或做任务获取' );
			return;
		}
		$curr_coin = $this->hincrCoin ( 0 - $award ['c'], "exchangeAward" );
		$status = 0;
		if ($award ['t'] == 3) {
			$status = 2;
			$msg = "兑换".$award ['na']."成功!";
			$curr_money = $this->hincrMoney($award ['ad'], "exchangeAward");
		}
		...
		$this->addAwardLog ( $this->uid, $this->param ['award_id'], $award ['c'],
				 $mob, 0, $award ['t'], $award ['na'],$status);
