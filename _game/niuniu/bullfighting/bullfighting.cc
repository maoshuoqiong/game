...
int main(int argc, char **argv)
{
    ...
    ret = single_instance_running(
        landlord.conf.get("pid_file", "conf/bullfighting.pid").asString().c_str());
    if (ret < 0) {
    ...
    log.start(landlord.conf["log"].get("log_file", "log/bullfighting.log").asString(),
