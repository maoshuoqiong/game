...
int RedisClient::connect_redis()
{
	...
	log.debug("redis host[%s] port[%d] is connected succ.\n", _host.c_str(), _port);
...
int RedisClient::get_lrange(std::vector<int> &vc)
{
	size_t i = 0;
	while (i < (reply->elements))
	{
		vc.push_back(::atoi(reply->element[i]->str));
		i += 1;
	}
	return 0;
}

void RedisClient::dump_elements()
{
	size_t i = 0;
	while (i < (reply->elements))
	{
	   log.debug("dump_elements[%s][%s]\n", reply->element[i]->str,reply->element[i+1]->str);
	   i += 2;
	}
}
