<?php
    ...
    class Lottery extends APIBase {
		...
		public function logic() {
			...
			$pExt = $this->data_redis->hMGet("hpe:{$this->uid}", array("telCharge",
					"lastLotteryDate","lastPrizeDate", "currLotteryNum"));
...
/* 			echo "###111##<br/>";
			...
			if(CommonTool::isVip($this->data_redis, $this->uid)>0){
				$remTime += 1;
			}
...
/* 
				...
				$lotteryRes = $this->calLotteryRes($this->uid,$lotteryPrize);
...
// 			var_dump($resArr);
		...
		public function payLottery($actType,$pExt,$remTime){
			...
			$cal = 0;
			$lotteryRes = -1;
			$identity = CommonTool::isVip($this->data_redis,$this->uid);
			...
			echo "###111##<br/>"; */
			if($actType == "lPrize"){	
				if(strtotime(Game::$lottery_activity_start)>$now
					|| strtotime(Game::$lottery_activity_end)<$now){
					return $this->returnData(array('start_t' =>strtotime(Game::$lottery_activity_start) ,
						'end_t' => strtotime(Game::$lottery_activity_end),'telCharge' => (int)$pExt['telCharge']));
				}
				...
				return $this->returnData(array('start_t' =>strtotime(Game::$lottery_activity_start) ,
						'end_t' => strtotime(Game::$lottery_activity_end),'telCharge' => (int)$pExt['telCharge'], 'res' => $lotteryPrize));
			}else if($actType == "lRes"){
				foreach ($prizeArr as $key) {
				...
				$mon = $this->data_redis->hMget("hu:{$this->uid}", array('money'));
				if(!isset($mon['money'])){
					$this->returnError(302, '非法操作');
					return;
				}
				...
				return $this->returnData(array('rem_num' => 0,'telCharge' => (int)$pExt['telCharge'],
						'curr_money' => (int)$mon['money'],'curr_coin' => (int)$mon['coin'],'res_ind' => $lotteryRes));
			}else{
				$this->returnError(302, '非法操作');
				return;
...
// 			echo "insert lottery log.$ret";
			...
			if(isset($lotteryItem['noticeId']) && isset($lotteryItem['desp']) && $lotteryItem['noticeId']>0){						
				$notice = $this->cache_redis->hMget("hnotice:{$lotteryItem['noticeId']}",array('type','desc'));
				$name = $this->data_redis->hMget("hu:{$this->uid}",array("name"));
				if(!isset($name['name'])){
					return;
				}
				// 			var_dump($notice);
				$msg = $lotteryItem['desc'];
				$old = array("{user}", "{desp}");
				$now   = array($name['name'], $lotteryItem['amount'].$lotteryItem['desp']);
				$msg = str_replace($old, $now, $msg);
				$this->sendBroadcast($msg,"lottery");
			}
...
// 				echo "BBB<br/>";
				$resArr = array('','','','','','','','','','');
				$identity = CommonTool::isVip($this->data_redis,$uid);
				$keys = $this->cache_redis->lRange("llt:ids", 0, -1);
				foreach ($keys as $key) {
					$temp = $this->cache_redis->hMGet("hlt:{$key}", array("awardType",
							"amount","sortInd","noticeId","userTypeProb","desp"));
					...
					$goods["userTypeProb"] = (int)$temp["userTypeProb"];
...
/* 			echo "###111##<br/>";
		...
		public function calLotteryRes($uid,$lotteryPrize){
			$identity = CommonTool::isVip($this->data_redis,$uid);
			$rand = mt_rand(1, 10000);
			$cal = 0;
// 			echo "@@rand:$rand";
